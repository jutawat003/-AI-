...
def suggest_careers(user_values: dict) -> list[tuple[str, str]]:
    ...
    ใช้ "สัดส่วนเงินเหลือ" (รายได้ - รายจ่าย) / รายได้ เป็นตัวตัดสินหลัก
    และใช้ระดับการยอมรับความเสี่ยง (ถ้ามีในโมเดล) มาช่วยเลือกอาชีพกลุ่มลงทุน
    """
    income = user_values.get("Monthly_Income")
    expense = user_values.get("Monthly_Expenditure")
    if income is None or expense is None or income <= 0:
        return []  # โมเดลนี้ไม่มีคอลัมน์รายรับ/รายจ่าย จึงแนะนำแบบเจาะจงไม่ได้

    surplus_ratio = (income - expense) / income   # สัดส่วนเงินเหลือต่อเดือน
    risk = user_values.get("Risk_Tolerance_Level")  # 0 (รับความเสี่ยงต่ำ) ถึง 1 (รับความเสี่ยงสูง)

    if surplus_ratio < 0.05:
        # รายได้กับรายจ่ายใกล้เคียงกันมาก หรือติดลบ ต้องการรายได้เสริมที่เริ่มได้เร็ว
        return [
            ("งานพาร์ทไทม์ที่เริ่มได้ทันที เช่น ไรเดอร์ส่งอาหาร/พัสดุ, งานตามชั่วโมง",
             "รายได้กับรายจ่ายใกล้เคียงกันมาก ควรเลือกงานที่เห็นรายได้เร็วและยืดหยุ่นเวลาได้"),
            ("รับจ้างฟรีแลนซ์ตามทักษะที่มี เช่น เขียนบทความ ออกแบบ แปลภาษา",
             "ไม่ต้องใช้เงินลงทุน เริ่มต้นได้จากทักษะเดิม ช่วยเพิ่มสภาพคล่องได้เร็ว"),
        ]
    elif surplus_ratio < 0.2:
        # มีเงินเหลือเล็กน้อย เหมาะกับอาชีพเสริมที่ใช้ทุนไม่มาก ยืดหยุ่นเวลา
        return [
            ("ขายของออนไลน์ หรือเป็นตัวแทนจำหน่ายสินค้า",
             "มีเงินเหลือเก็บอยู่บ้าง เหมาะกับอาชีพเสริมที่ใช้ทุนไม่มากและทำนอกเวลางานได้"),
            ("รับสอนพิเศษ หรือให้คำปรึกษาตามความถนัด",
             "ใช้เวลาว่างบางส่วนสร้างรายได้เพิ่มโดยไม่กระทบงานประจำ"),
        ]
    else:
        # มีเงินเหลือค่อนข้างมาก เลือกได้ตามระดับความเสี่ยงที่ยอมรับได้
        options = [
            ("รับสอนพิเศษ หรือเป็นที่ปรึกษาตามความเชี่ยวชาญ",
             "มีรายได้ส่วนเกินเพียงพอ เหมาะกับงานเสริมที่ต่อยอดความรู้ความเชี่ยวชาญของตัวเอง"),
        ]
        if risk is not None and risk >= 0.5:
            options.append((
                "ลงทุนสร้างรายได้แบบ Passive เช่น หุ้น กองทุนรวม หรือปล่อยเช่าสินทรัพย์",
                "มีเงินเหลือเก็บและยอมรับความเสี่ยงได้ในระดับหนึ่ง เหมาะกับการต่อยอดเป็นรายได้เสริมระยะยาว",
            ))
        else:
            options.append((
                "รับงานฟรีแลนซ์ตามทักษะที่ถนัด ทำเป็นรายได้เสริมระยะยาว",
                "ฐานะการเงินมั่นคง สามารถเลือกงานเสริมที่ไม่ต้องแบกรับความเสี่ยงสูงได้",
            ))
        return options
    ...
    for group_title, names in groups:
        ...
        for idx, name in enumerate(names_in_model):
            ...
            if var.is_discrete:
                ...
                # คอลัมน์ชนิดตัวเลข → number_input พิมพ์ค่าเองได้โดยตรง
                # (ยังมีปุ่ม +/- ให้ปรับทีละสเต็ปได้ด้วยถ้าไม่อยากพิมพ์)
                ...
                user_values[var.name] = box.number_input(
                    ...
                    format=fmt,
                    key=f"in_{var.name}",
                    help=f"ช่วงข้อมูลที่ใช้ฝึกโมเดล: {lo:,.2f} – {hi:,.2f} (กรอกค่านอกช่วงนี้ได้ แต่ผลทำนายอาจแม่นยำน้อยลง)",
                )
...
if submitted:
    ...
    if careers:
        ...
        st.caption(
            "แนะนำจากสัดส่วนรายรับ-รายจ่าย (และระดับความเสี่ยงที่ยอมรับได้ ถ้ามีข้อมูล) "
            "ไม่ใช่ผลที่โมเดลทำนายโดยตรง"
        )
        for job_name, reason in careers:
